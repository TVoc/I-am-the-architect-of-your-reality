...
\begin{itemize}
    ...
    Provides methods to register new recipients and new Customer Organizations (and their Customer Administrators). Upon successful registration, a login token is generated so that the user is immediately logged in. If an account with the supplied registration information (company name for Customer Organizations, e-mail address for recipient) already exists, the registration process is terminated and an exception is raised. If a new recipient is registered, it is indicated to the \texttt{ReplicationManager} that all documents addressed to that recipient must now be stored in the personal document storage.
